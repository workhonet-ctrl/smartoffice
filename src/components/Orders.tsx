...
export default function Orders({ onImportDone }: { onImportDone?: (ids: string[]) => void }) {
  ...
  const handleLockOrderCost = async () => {
    if (!costLockTarget || costLocking) return;

    setCostLocking(true);
    try {
      const { data, error } = await supabase.rpc('lock_order_cost_fifo', {
        p_order_id: costLockTarget.id,
      });

      if (error) throw error;

      const result: any = data || {};
      const costTotal = Number(result.cost_total_thb || 0);
      const avgCost = Number(result.cost_per_unit_avg || 0);

      showToast(
        result.already_locked
          ? `ออเดอร์ ${costLockTarget.order_no} ล็อกต้นทุนไว้แล้ว`
          : `✓ ล็อกต้นทุน ${costLockTarget.order_no} สำเร็จ · ต้นทุนรวม ฿${costTotal.toLocaleString('th-TH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} · เฉลี่ย ฿${avgCost.toLocaleString('th-TH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
        'success'
      );

      setCostLockTarget(null);
      await loadOrders();
    } catch (err: any) {
      showToast('ล็อกต้นทุนไม่สำเร็จ: ' + (err.message || 'unknown'), 'error');
    } finally {
      setCostLocking(false);
    }
  };
  ...
  return (
    <div className="flex flex-col h-screen p-3 sm:p-6 pb-2">
      ...
      {activeTab === 'orders' && (
      <div className="flex-1 bg-white rounded-xl shadow overflow-auto min-h-0">
        <table className="text-sm" style={{ minWidth: '1240px', width: '100%' }}>
          <thead className="bg-slate-800 text-slate-200 text-xs sticky top-0 z-10">
            <tr>
              <th className="p-3 w-8">
                <input type="checkbox"
              ...
              <th className="p-3 text-center whitespace-nowrap text-emerald-300">ต้นทุน</th>
          ...
          <tbody>
            ...
            {filtered.length === 0 && orders.length > 0 && (
              <tr><td colSpan={17} className="p-8 text-center text-slate-400">ไม่พบออเดอร์ที่ตรงกับตัวกรอง</td></tr>
            )}
              {pagedOrders.map(o => {
                      ...
                      {(o as any).cost_locked ? (
                        <div className="flex flex-col items-center gap-0.5">
                          ...
                          <span className="text-[11px] font-bold text-emerald-700">
                            ฿{Number((o as any).cost_total_thb || 0).toLocaleString('th-TH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                          </span>
                        </div>
                      ) : (
                        <button onClick={() => setCostLockTarget(o)}
                          className="px-2 py-1 rounded-lg bg-amber-50 text-amber-700 hover:bg-amber-100 text-[11px] font-bold border border-amber-100">
                          🔐 ล็อกต้นทุน
                        </button>
                      )}
                    </td>
        ...
        <div className="fixed inset-0 bg-black/45 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-[2rem] w-full max-w-md shadow-2xl border border-emerald-100 overflow-hidden">
            <div className="bg-gradient-to-br from-emerald-50 via-cyan-50 to-amber-50 px-6 py-6 border-b border-emerald-100 relative">
              ...
              <h3 className="text-xl font-extrabold text-slate-800">ล็อกต้นทุนออเดอร์นี้ใช่ไหม?</h3>
              <p className="text-sm text-slate-500 mt-2 leading-6">
                ระบบจะตัดล็อตต้นทุนแบบ FIFO และบันทึก snapshot ลงออเดอร์ เพื่อให้กำไรย้อนหลังไม่เปลี่ยน
              </p>
            </div>

            <div className="px-6 py-4 space-y-3">
              <div className="rounded-2xl bg-slate-50 border border-slate-100 p-3 text-sm">
                <div className="flex justify-between gap-3"><span className="text-slate-400">เลขออเดอร์</span><b className="font-mono">{costLockTarget.order_no}</b></div>
                <div className="flex justify-between gap-3 mt-1"><span className="text-slate-400">ลูกค้า</span><b>{costLockTarget.customers?.name || '-'}</b></div>
                <div className="flex justify-between gap-3 mt-1"><span className="text-slate-400">ยอดขาย</span><b>฿{Number(costLockTarget.total_thb || 0).toLocaleString()}</b></div>
                <div className="mt-2 pt-2 border-t border-slate-200 text-xs text-slate-500">
                  {(costLockTarget.raw_prod || '').split('|').filter(Boolean).join(' · ') || '-'}
                </div>
              </div>

              <div className="rounded-2xl bg-amber-50 border border-amber-100 px-3 py-2 text-xs text-amber-700 leading-5">
                ⚠️ หลังยืนยัน ระบบจะหักจำนวนคงเหลือของล็อตต้นทุนจริง และออเดอร์นี้จะเปลี่ยนเป็น “ล็อกแล้ว”
              </div>
            </div>
            ...
            <div className="px-6 pb-6 flex justify-end gap-2">
              ...
              <button onClick={handleLockOrderCost} disabled={costLocking}
                className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-500 text-white hover:from-emerald-600 hover:to-cyan-600 font-bold shadow disabled:opacity-50">
                {costLocking ? 'กำลังล็อก...' : 'ยืนยันล็อกต้นทุน'}
              </button>
            </div>
          </div>
        </div>
